...
colorMap = {EMPTY: (0, 0, 0, 255), FLOOR: (255, 128, 0, 255), CORRIDOR:(255, 164, 0, 255),
            DOOR: (0, 128, 0, 255), DEADEND: (128, 0, 128, 255), WALL:(128, 128, 128, 255),
            OBSTACLE: (128, 0, 0, 255), CAVE: (0, 0, 128, 255)}

colorMapR = {(0, 0, 0, 255): [EMPTY], (255, 128, 0, 255): [FLOOR], (255, 164, 0, 255): [CORRIDOR],
            (0, 128, 0, 255): [DOOR], (128, 0, 128, 255): [DEADEND], (128, 128, 128, 255): [WALL],
            (128, 0, 0, 255): [OBSTACLE], (0, 0, 128, 255): [CAVE], (255, 255, 255, 255): [WILDCARD],
            (0, 128, 255, 255): [BADTILE], (64, 64, 64, 255): [WALL, EMPTY]}
"""
# Enum might not be important
wallCodesAnfang = 0xA0
lWallRUT = 0xA0 # L-shaped wall that goes right then up like so _|
lWallLUT = 0xA1
lWallRDT = 0xA2
lWallLDT = 0xA3
wallCornerLUT = 0xA4 # Forms corners of walls, generally quite small
wallUT = 0xA5
wallCornerRUT = 0xA6
doorUT = 0xA7
wallLT = 0xA8
...
wallCornerRDT = 0xAE
doorLT = 0xAF
# Note: 0xB0 - 0xB2 are available constants
doorRT = 0xB3
lcWallRUT = 0xB4
lcWallLUT = 0xB5
lcWallRDT = 0xB6
...
cWallVLDT = 0xC7
"""  
tileset = Image.open("basic_tileset.png")
tiles = []
k = 0
for j in range(tileset.height):    
    for i in range(tileset.width):
        if i % 24 == 0 and j % 24 == 0:
            cropped = tileset.crop((i, j, i+24, j+24))
            #tImg = Image.new("RGB", (cropped.width, cropped.height))
            #tImg.paste(cropped, (0, 0, tImg.width, tImg.height))
            #cropped.save("output/%i_tileset.png" % k)
            tiles.append(cropped)
            k += 1

def exportTileset(modifier="test"):
    for tile in range(len(tiles)):
        tileImg = tiles[tile]
        tileImg.save("output/%s_%i_tileset.png" % (modifier, tile))

keys = {}
def parseKey():
    key = Image.open("mapParseKey.png")
    for i in range(key.width):
        for j in range(key.height):
            if i % 3 == 0 and j % 3 == 0:
                keyIndex = i/3 + ((j/3)*4)
                keys[keyIndex] = [None] * 9
                keys[keyIndex][0] = colorMapR[key.getpixel((i, j))] # Top Left
                keys[keyIndex][1] = colorMapR[key.getpixel((i+1, j))] # Top
                keys[keyIndex][2] = colorMapR[key.getpixel((i+2, j))] # Top Right
                keys[keyIndex][3] = colorMapR[key.getpixel((i, j+1))] # Left
                keys[keyIndex][4] = colorMapR[key.getpixel((i+1, j+1))] # Center
                keys[keyIndex][5] = colorMapR[key.getpixel((i+2, j+1))] # Right
                keys[keyIndex][6] = colorMapR[key.getpixel((i, j+2))] # Bottom Left
                keys[keyIndex][7] = colorMapR[key.getpixel((i+1, j+2))] # Bottom
                keys[keyIndex][8] = colorMapR[key.getpixel((i+2, j+2))] # Bottom Right
    #print(keys)
parseKey()
class dungeonRoom:
    """ 
...
class dungeonGenerator:
    ...
    def toTiledMap(self, gid):
        ...
        for j in range(self.height):
            ...
            for i in range(self.width):
                eTile = self.parseTileTypeFromKey(i, j)
                if eTile:
                    #print(len(grid), i)
                    grid[j].append(gid + eTile)
                else:
                    grid[j].append(0)
        #print(grid)
        return grid
    
    def exportMinimap(self, fn="minimap.png"):
        img = Image.new("RGB", (self.width, self.height))
        for i in range(self.width):
            for j in range(self.height):
                tile = self.grid[i][j]
                img.putpixel((i, j), colorMap[tile])
        img.save(fn)
    
    def parseNeighbours(self, x, y, fn):
        neighbours = self.findNeighbours(x, y)
        eTileImg = Image.new("RGB", (3*24, 3*24))
        eMiniTileImg = Image.new("RGB", (3, 3))
        imgs = []
        colors = []
        for i, j in neighbours:
            #print("Next")
            parsed = self.parseTileTypeFromKey(i, j)
            if parsed:
                imgs.append(tiles[parsed])
            else:
                imgs.append(None)
            colors.append(colorMap[self.grid[i][j]])
        x = 0
        y = 0
        #print(imgs)
        for i in range(len(imgs)):
            #print(x, y, eMiniTileImg.width, eMiniTileImg.height, colors)
            if imgs[i]:
                eTileImg.paste(imgs[i], (x*24, y*24, (x+1)*24, (y+1)*24))
            eMiniTileImg.putpixel((x, y), colors[i])
            x += 1
            if x >= 3:
                x = 0
                y += 1
        eTileImg.save("test/%s_tileImg.png" % fn)
        eMiniTileImg.save("test/%s_miniTileImg.png" % fn)

    def exportLightingPreview(self, fn="preview.png"):
        img = Image.new("RGBA", (self.width*24, self.height*24))
        img.paste((0, 0, 0, 255), (0, 0, self.width*24, self.height*24))
        greyOverlay = Image.open("grey_overlay.png")
        for i in range(self.width):
            for j in range(self.height):
                eTile = self.parseTileTypeFromKey(i, j)
                if eTile:
                    tileImg = tiles[eTile]
                    img.paste(tileImg, (i*24, j*24, (i*24)+24, (j*24)+24))
                    img.paste(greyOverlay, (i*24, j*24, (i*24)+24, (j*24)+24), greyOverlay)
        img.save(fn)

    def exportPreview(self, fn="preview.png"):
        img = Image.new("RGB", (self.width*24, self.height*24))
        for i in range(self.width):
            for j in range(self.height):
                eTile = self.parseTileTypeFromKey(i, j)
                if eTile:
    ...
    def parseTileTypeFromKey(self, x, y):
        #neighbours = self.findNeighbours(x, y)
        if x-1 > 0 and y-1 > 0:
            ulTile = self.grid[x-1][y-1]
        else:
            ulTile = WALL
        if y-1 > 0:
            #print(y, y-1 > 0)
            uTile = self.grid[x][y-1]
        else:
            uTile = WALL
        if x+1 < self.width and y-1 > 0:
            urTile = self.grid[x+1][y-1]
        else:
            urTile = WALL
        if x-1 > 0:
            lTile = self.grid[x-1][y]
        else:
            lTile = WALL
        if x+1 < self.width:
            rTile = self.grid[x+1][y]
        else:
            rTile = WALL
        if x-1 > 0 and y+1 < self.height:
            dlTile = self.grid[x-1][y+1]
        else:
            dlTile = WALL
        if y+1 < self.height:
            dTile = self.grid[x][y+1]
        else:
            dTile = WALL
        if x+1 < self.width and y+1 < self.height:
            drTile = self.grid[x+1][y+1]
        else:
            drTile = WALL
        centerTile = self.grid[x][y]
        for var in ["ulTile", "uTile", "urTile", "lTile", "centerTile", "rTile", "dlTile", "dTile", "drTile"]:
            if eval(var) == CORRIDOR:
                exec("%s = FLOOR" % var)
            if var != "centerTile" and eval(var) == DOOR:
                exec("%s = FLOOR" % var)
                #print("%s = FLOOR" % var)
        #print(ulTile, uTile, urTile, lTile, centerTile, rTile, dlTile, dTile, drTile)
        for keyIndex in keys:
            #print(keyIndex)
            key = keys[keyIndex]
            #if key[0] == ulTile and key[1] == uTile:
            #    print("Matches!", urTile)
            if (ulTile in key[0] or WILDCARD in key[0]) and\
                (uTile in key[1] or WILDCARD in key[1]) and\
                (urTile in key[2] or WILDCARD in key[2]) and\
                (lTile in key[3] or WILDCARD in key[3]) and\
                (centerTile in key[4] or WILDCARD in key[4]) and\
                (rTile in key[5] or WILDCARD in key[5]) and\
                (dlTile in key[6] or WILDCARD in key[6]) and\
                (dTile in key[7] or WILDCARD in key[7]) and\
                (drTile in key[8] or WILDCARD in key[8]):
                #print("KeyIndex: ", keyIndex)
                return keyIndex

    #### Deprecated Grawn Functions ####
    def parseWallTileType(self, x, y):
        if x-1 > 0 and y-1 > 0:
            ulTile = self.grid[x-1][y-1]
        else:
            ulTile = None
        ...
        if centerTile == FLOOR:
            return floorT
        elif centerTile == WALL:
            if ulTile == FLOOR and uTile == FLOOR and urTile == FLOOR and\
                lTile == WALL and rTile == WALL and\
                dlTile == FLOOR and dTile == FLOOR and drTile == FLOOR:
                return dWallHT
            elif ulTile == FLOOR and uTile == WALL and urTile == FLOOR and\
                lTile == FLOOR and rTile == FLOOR and\
                dlTile == FLOOR and dTile == WALL and drTile == FLOOR:
                return dWallVT
            elif ulTile == FLOOR and uTile == FLOOR and urTile == FLOOR and\
                lTile == WALL and rTile == FLOOR and\
                dlTile == FLOOR and dTile == FLOOR and drTile == FLOOR:
                return uWallHLT
            elif ulTile == FLOOR and uTile == FLOOR and urTile == FLOOR and\
                lTile == FLOOR and rTile == WALL and\
                dlTile == FLOOR and dTile == FLOOR and drTile == FLOOR:
                return uWallHRT
